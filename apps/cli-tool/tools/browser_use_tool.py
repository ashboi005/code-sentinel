...
async def run_browser_agent(
    task: str, headless: bool, use_vision: bool, screenshot_path: str | None
) -> dict:
    ...
    agent = Agent(task=task, llm=llm, browser=browser, use_vision=use_vision)
    ...
    try:
        ...
        return {
            "status": "success",
            "message": result_text
            or "Agent task completed with no explicit final message.",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CodeSentinel Browser Use Automation Tool"
    )
    parser.add_argument(
        "--task", required=True, help="Instruction/task for the browser agent"
    )
    parser.add_argument(
        "--headless", action="store_true", default=True, help="Run browser headlessly"
    )
    parser.add_argument(
        "--no-headless",
        action="store_false",
        dest="headless",
        help="Run browser visually (headful)",
    )
    parser.add_argument(
        "--use-vision",
        action="store_true",
        default=False,
        help="Enable vision/screenshots for LLM reasoning",
    )
